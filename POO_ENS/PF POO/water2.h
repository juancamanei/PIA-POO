...
class Water2 : public Primitivos, public Imagenes
{
...
public:
	...
	Water2(HWND hWnd, WCHAR alturas[], WCHAR textura[], float ancho, float prof)
	{
		anchof = ancho;
		proff = prof;
	...
	{
		static float MovS = 0;
		MovS += .02;//velocidad sube baja
		float A = sin(MovS) * 0.003;//amplitud
		//glPushAttrib(GL_CURRENT_BIT | GL_TEXTURE_BIT);
		//glEnable(GL_DEPTH_TEST);
		glEnable(GL_BLEND);

		static float water_factor_min = 0.0f;
		static float water_factor_max = 18.0f;
		static float water_size = 60;
		...
		//quiero saber para que son estos
		//mueve de lado a lado
		// la A es un seno
		water_factor_min += 1 * A;
		water_factor_max += 1 * A;
